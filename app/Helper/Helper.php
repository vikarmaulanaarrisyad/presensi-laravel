...
if (!function_exists('uploadBase64')) {
    function uploadBase64($directory, $base64Image, $filename = '')
    {
        $imageParts = explode(";base64,", $base64Image);
        $imageType = str_replace('data:image/', '', $imageParts[0]);
        $imageBase64 = base64_decode($imageParts[1]);

        $filename = ($filename ?: uniqid()) . '_' . date('YmdHis') . '.' . $imageType;

        Storage::disk('public')->put("$directory/$filename", $imageBase64);
